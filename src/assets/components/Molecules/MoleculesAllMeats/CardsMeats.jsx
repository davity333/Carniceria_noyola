...
function CardsMeats({ src, productName, price, children, onClick, amount }) {
  ...
  return (
    ...
    >
      ...
      {isOutOfStock ? (
        <div className="mt-4 sm:mt-6 md:mt-8 p-2 sm:p-3 md:p-4 rounded-lg bg-red-600 text-white">
          <p className="text-[10px] sm:text-[15px] md:text-xl lg:text-2xl">Agotado</p>
        </div>
      ) : (
        <div className="cursor-pointer relative mt-4 sm:mt-6 md:mt-8 rounded-lg flex items-center justify-center p-2 sm:p-3 md:p-4 group bg-slate-700 overflow-hidden" onClick={onClick}>
