...
function ProductFoundOrg({nameFound, src, nameFoundApi, array,onCardClick}) {
    ...
    return ( 
        ...
        <div className="flex justify-center p-5 text-3xl text-[#992f2f]">
            ...
            <div className="p-9">
                {pluralText && (<p className="text-5xl [text-shadow:_0px_3px_3px_#6f6262] text-center">Productos similares a</p>)}
                {singularText && (<p className="text-5xl [text-shadow:_0px_3px_0px_rgba(111,98,98,0.44)] text-center">Producto similiar a</p>)}
                {names && (<p className="flex justify-center text-[#937122] text-4xl [text-shadow:_-2px_1px_0px_rgba(212,191,191,0.44)]">{nameFound}</p>)}

                    {productNoFound &&(<div>
                    <p className="text-5xl m-10 text-[#702929] [text-shadow:_0px_3px_3px_#6f6262] text-center">Producto no encontrado</p>
                    <div className="flex justify-center">
                    <img src={src} alt="logo" className="h-32"/>
                    </div>
                    <p className="text-black flex justify-center m-6 text-[#322f2f]
                    [text-shadow:_0px_3px_3px_#6f6262] text-center">Este producto no existe</p>
                    <p className="flex justify-center text-[#9f3232] text-4xl ">{nameFound}</p>
                    <p className="flex justify-center text-[0.7em] text-[#322f2f]
                    font-light sm:text-2xl text-center">Lo sentimos, pero no tenemos disponible este tipo</p>
        ...
        <div className="grid grid-cols-2  justify-center ">
            {products.map((product, index) => (
                <CardsMeats
                    ...
                    onClick={() => onCardClick(product)} // Utilizar la prop onCardClick
                />       
            ))}
        </div>
    </div>
